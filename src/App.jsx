...
import {
  ArrowLeft, ArrowRight, ArrowUpRight, Check, ChevronDown, FileText,
...
function AboutPage() {
  ...
  const scroll=(ref,direction)=>ref.current?.scrollBy({left:direction*420,behavior:'smooth'})
  ...
  return <Shell className="about-page"><section className="section about-intro"><h1>Производитель<br/>профессиональной<br/>строительной химии</h1><div className="about-intro__visual"><img src={`${A}figma/figma-hero.png`} alt="Герметик KLEIM PRO"/><article className="about-years"><strong>20+</strong><b>лет</b><p>Разрабатываем и производим герметики, клеи, монтажные пены и специализированные составы для строительства, промышленности и профессионального ремонта.</p></article><article className="about-product-note"><img src={`${A}logo.svg`} alt="KLEIM PRO"/><p>Профессиональная линейка компании Master Klein, объединяющая современные технологии, собственное производство и строгий контроль качества.</p><Link to="/wholesale">Оптовым клиентам и дилерам <span>↗</span></Link></article></div></section>
    ...
    <section className="section about-quality"><h2>Система контроля<br/>качества</h2><div className="about-quality__media"><div><img src={`${A}about-certificate.jpg`} alt="Сертификат ISO"/><img src={`${A}about-certificate.jpg`} alt="Сертификат ISO"/></div><figure><img src={`${A}raw-10.jpg`} alt="Контроль качества на производстве"/><figcaption><strong>680</strong><span>Аудитов</span></figcaption></figure></div><div className="about-quality__points">{[['01','Контроль над маршрутами','Является одним из ключевых факторов, усиливающих скорость доставки и повышающих эффективность логистических процессов.'],['02','Международные сертификаты','Наличие международных сертификатов ISO 9001 и ГОСТ Р подтверждает соответствие продукции строгим требованиям отрасли.'],['03','Надёжность продукции','В 2023 году проведено 680 аудитов, по итогам которых количество рекламаций не превысило 0,25%.']].map(([n,title,text])=><article key={n}><span>{n}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section className="section about-fleet"><img src={`${A}about-fleet.jpg`} alt="Собственный автопарк KLEIM PRO"/><div className="about-fleet__intro"><h2>Собственный<br/>автопарк</h2><p>Является одним из ключевых факторов, усиливающих скорость доставки и повышающих эффективность логистических процессов.</p></div><div className="about-fleet__note"><b>Контроль перевозок</b><p>Владение собственным парком транспортных средств предоставляет компании полный контроль над маршрутами и графиками отгрузок.</p></div></section>
    <section className="section about-map"><img src={`${A}map.png`} alt="Карта присутствия KLEIM PRO"/></section>
    <section className="section about-contribution"><div className="section-heading"><h2>Вклад в развитие<br/>отрасли</h2><div className="slider-buttons"><button onClick={()=>scroll(contributionRef,-1)}><ArrowLeft/></button><button onClick={()=>scroll(contributionRef,1)}><ArrowRight/></button></div></div><div className="about-scroll-track" ref={contributionRef}>{contributions.map(([image,title])=><article key={title}><img src={`${A}${image}`} alt=""/><div><h3>{title}</h3><p>Активное участие в профессиональных инициативах способствует развитию инноваций и расширению профессиональных контактов.</p></div></article>)}</div></section>
    <BlogSection/>
    <section className="section about-team"><div className="about-team__side"><h2>Наша<br/>команда</h2><div className="slider-buttons"><button onClick={()=>scroll(teamRef,-1)}><ArrowLeft/></button><button onClick={()=>scroll(teamRef,1)}><ArrowRight/></button></div></div><div className="about-team__track" ref={teamRef}>{team.map(([image,name,role],index)=><article key={`${name}-${index}`}><img src={`${A}${image}`} alt={name}/><div><h3>{name}</h3><span>{role}</span></div></article>)}</div></section>
  </Shell>
}
...
function DocumentGrid({ small=false }){
  ...
  return <div className={`document-grid ${small?'document-grid--small':''}`}>{docs.map((d,i)=><a href={`${A}pages/documents/${(i%8)+1}.png`} download key={d}><FileText/><span>{d}</span><ArrowUpRight/></a>)}</div>
}
...
function ArticlePageNew(){
  const workItems=[
    'Герметизация малоподвижных соединений в бетонных и кирпичных конструкциях.',
    'Заполнение небольших трещин и швов в штукатурке и древесине.',
    'Оформление примыканий между стенами, перегородками и потолком.',
    'Работы внутри помещений с последующим окрашиванием поверхности.',
    'Уплотнение соединений, где не требуется постоянная эластичность.',
    'Монтажные и отделочные работы при соблюдении температурного режима.'
  ]
  return <Shell><main className="article-redesign">
    ...
    <article className="section article-redesign__body">
      ...
      <aside className="article-redesign__notice"><strong>!</strong><p>Статья — жанр журналистики, в котором автор ставит задачу проанализировать общественные ситуации, процессы и явления. В статье автор рассматривает отдельные ситуации как часть более широкого явления и аргументированно пишет о своей точке зрения.</p></aside>
      ...
      <blockquote className="article-redesign__quote"><span>“</span><p>Нераспечатанные картриджи необходимо хранить в оригинальной упаковке и соблюдать температурные условия, указанные производителем. Герметик следует защищать от прямых солнечных лучей и сильного нагрева.</p><footer><b>Иван Варламов</b><small>Ген. директор</small></footer></blockquote>
      ...
      <section className="article-redesign__avoid"><h2>Где не стоит использовать<br/>акриловый герметик</h2><p>Акриловый герметик A201 подходит для распространённых строительных оснований, но некоторые условия требуют другого типа состава.</p><div className="article-redesign__avoid-main"><strong>01</strong><b>Для работы на улице важна защита от ветра и влаги. Материал должен сохранять свойства при высокой физической и температурной нагрузке.</b></div><ol><li>Не наносите герметик на загрязнённую или мокрую поверхность.</li><li>Не используйте состав в постоянно подвижных соединениях и местах длительного контакта с водой.</li><li>Не заполняйте слишком глубокие швы без уплотнительного шнура.</li></ol></section>
    </article>
  </main></Shell>
}
